...
class SVCompDriver:
        ...
        if status == ExecutionStatus.SUCCESS:
            ...
            for l in output:
                ...
                if "java.lang.AssertionError" in l:
                    ...
                    return Action.REPORTVERDICT
                if "[SWAT] Uncaught top-level exception in symbolic execution" in l:
    ...
    def retrieve_solution(self):
        ...
        #logger.debug(f'[SYMBOLIC EXPLORATION] Possible branches: {", ".join([str(b.constraint) for b in possible_branches])}')
        symbolic_vars = None
        sat = None
        branch_found = False
        for branch in possible_branches:
            #logger.debug(f'[SYMBOLIC EXPLORATION] Checking branch {branch.id}: {branch.constraint}')
            if not StrategyService.is_symbolic_branch(branch):
                #logger.debug(f'[SYMBOLIC EXPLORATION] Skipping non-symbolic branch {branch.id}: {branch.constraint}')
                continue
            branch_found = True
