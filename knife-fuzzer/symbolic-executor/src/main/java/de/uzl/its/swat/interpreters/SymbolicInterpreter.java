...
import de.uzl.its.swat.invoke.DynamicInvocation;
import de.uzl.its.swat.invoke.StaticInvocation;
import de.uzl.its.swat.logger.ClassNames;
import de.uzl.its.swat.logger.FieldInfo;
import de.uzl.its.swat.logger.ObjectInfo;
import de.uzl.its.swat.logger.SystemLogger;
import de.uzl.its.swat.logger.inst.*;
...
import de.uzl.its.symbolic.value.reference.LambdaPlaceHolder;
...
public class SymbolicInterpreter implements IVisitor {
    ...
    private final SymbolicStateHandler symbolicStateHandler;
    ...
    public SymbolicInterpreter(ClassNames classNames) {
        stack = new Stack<>();

        stack.add(currentFrame = new Frame(0));
        this.classNames = classNames;
        objects = new HashMap<>();
        symbolicStateHandler = new SymbolicStateHandler();
    ...
    public SymbolicStateHandler getSymbolicStateHandler() {
        return symbolicStateHandler;
    }
    ...
    public void visitBALOAD(BALOAD inst) {
        try {
            ...
            if (arr instanceof BooleanArrayValue barr) {
                ...
                systemLogger.warn("[BALOAD]: Unknown array type");
                currentFrame.push(
                        arr.getName() != null
                                ? PlaceHolder.symbolicInstance
                                : PlaceHolder.instance);
    ...
    public void visitBASTORE(BASTORE inst) {
        try {
            ...
            if (ref instanceof BooleanArrayValue arr) {
                ...
                systemLogger.warn("[BASTORE]: Unknown array type");
            }
        } catch (Exception e) {
            throwError(inst, e);
        }
    ...
    public void visitCALOAD(CALOAD inst) {
        try {
            ...
            if (arr instanceof CharArrayValue carr) {
                ...
                systemLogger.warn("[CALOAD]: Unknown array type");
                currentFrame.push(
                        arr.getName() != null
                                ? PlaceHolder.symbolicInstance
                                : PlaceHolder.instance);
    ...
    public void visitCASTORE(CASTORE inst) {
        try {
            ...
            if (ref instanceof CharArrayValue arr) {
                ...
                systemLogger.warn("[CASTORE]: Unknown array type");
            }
        } catch (Exception e) {
            throwError(inst, e);
        }
    ...
    public void visitDALOAD(DALOAD inst) {
        try {
            ...
            if (arr instanceof DoubleArrayValue darr) {
                ...
                systemLogger.warn("[DALOAD]: Unknown array type");
                currentFrame.push(
                        arr.getName() != null
                                ? PlaceHolder.symbolicInstance
                                : PlaceHolder.instance);
    ...
    public void visitDASTORE(DASTORE inst) {
        try {
            ...
            if (ref instanceof DoubleArrayValue arr) {
                ...
                systemLogger.warn("[DASTORE]: Unknown array type");
            }
        } catch (Exception e) {
            throwError(inst, e);
        }
    ...
    public void visitFALOAD(FALOAD inst) {
        try {
            ...
            if (arr instanceof FloatArrayValue farr) {
                ...
                systemLogger.warn("[FALOAD]: Unknown array type");
                currentFrame.push(
                        arr.getName() != null
                                ? PlaceHolder.symbolicInstance
                                : PlaceHolder.instance);
    ...
    public void visitFASTORE(FASTORE inst) {
        try {
            ...
            if (ref instanceof FloatArrayValue arr) {
                ...
                systemLogger.warn("[FASTORE]: Unknown array type");
            }
        } catch (Exception e) {
            throwError(inst, e);
        }
    ...
    public void visitGETFIELD(GETFIELD inst) {
        try {
            ...
            Value val;
            if (ref.getAddress() == 0) {
                systemLogger.warn("GETFIELD on NULL object");
    ...
    public void visitIALOAD(IALOAD inst) {
        try {
            ...
            if (ref instanceof IntArrayValue arr) {
                ...
                systemLogger.warn("[IALOAD]: Unknown array type");
                currentFrame.push(
                        ref.getName() != null
                                ? PlaceHolder.symbolicInstance
                                : PlaceHolder.instance);
    ...
    public void visitIASTORE(IASTORE inst) {
        try {
            ...
            if (ref instanceof IntArrayValue arr) {
                ...
                systemLogger.warn("[IASTORE]: Unknown array type");
            }
        } catch (Throwable t) {
            throwError(inst, t);
        }
    ...
    public void visitIFEQ(IFEQ inst) {
        ...
        /* if (!loops.isEmpty() && isBranchTaken) {
            systemLogger.info("[LOOP EXIT] Branch instruction iid: " + inst.iid);
    ...
    private void setArgumentsAndNewFrame(
        ...
        if (owner.equals("java/net/Socket")) {
            systemLogger.info("Socket mocking not implemented: " + owner + "/" + name);
    ...
    public void visitLALOAD(LALOAD inst) {
        try {
            ...
            if (arr instanceof LongArrayValue larr) {
                ...
                systemLogger.warn("[LALOAD]: Unknown array type");
                currentFrame.push(
                        arr.getName() != null
                                ? PlaceHolder.symbolicInstance
                                : PlaceHolder.instance);
    ...
    public void visitLASTORE(LASTORE inst) {
        try {
            ...
            if (ref instanceof LongArrayValue arr) {
                ...
                systemLogger.warn("[LASTORE]: Unknown array type");
            }
        } catch (Exception e) {
            throwError(inst, e);
        }
    ...
    public void visitSALOAD(SALOAD inst) {
        try {
            ...
            if (arr instanceof ShortArrayValue sarr) {
                ...
                systemLogger.warn("[SALOAD]: Unknown array type");
                currentFrame.push(
                        arr.getName() != null
                                ? PlaceHolder.symbolicInstance
                                : PlaceHolder.instance);
    ...
    public void visitSASTORE(SASTORE inst) {
        try {
            ...
            if (ref instanceof ShortArrayValue arr) {
                ...
                systemLogger.warn("[SASTORE]: Unknown array type");
            }
        } catch (Exception e) {
            throwError(inst, e);
        }
    ...
    public void visitLOOP_END(LOOP_END inst) {
        /*
        systemLogger.info("[LOOP END] Branch instruction iid: " + inst.iid);
        int iid = loops.pop();
        assert iid == inst.iid;
