...
public class Util {
    ...
    public static boolean isMatchingType(Type type, Value<?, ?> val) throws NoThreadContextException {
        if (val == ValueFactory.createNULLValue())
            return true;
    ...
    public static Value<?, ?> convertType(Type type, Value<?, ?> val)
            throws NoThreadContextException, NotImplementedException, ValueConversionException {
        if (val == ValueFactory.createNULLValue())
            return val;
        String descriptor = type.getDescriptor();
        return switch (descriptor) {
    ...
    /**
     * Checks if a class should be instrumented. The decision is based on both
     * include and exclude
     * packages, with priority given to the most specific (longest matching prefix)
     * package.
     *
     * <p>
     * If the include packages are set, only classes in those packages will be
     * instrumented. If
     * the exclude packages are set, classes in those packages will not be
     * instrumented. If a class
     * matches both an include and an exclude package, the more specific match
     * (longer prefix) will
     * take precedence.
     *
    ...
    public static boolean shouldInstrument(String cname) {
        ...
        // logger.trace("Decision to instrument '{}': Included prefix = {}, Excluded
        // prefix = {}, Verdict = {}",
    ...
    public static Field[] getDeclaredFields(Class<?> clazz)
        ...
        // Get fields, this might cause a class load, which might trigger an
        // instrumented class
     ...
     * This is intended for classes that are enhanced or proxied by frameworks like
     * Hibernate or Spring.
     * Todo: This is a very simple implementation and does not cover all cases. In
     * particular, classes are allowed to
    ...
    /**
     * Loads a class for a given name and disables the logging during the load.
     * Currently it exits
    ...
    public static Class<?> loadClassForName(String className)
            throws NoThreadContextException, ThreadAlreadyDisabledException, ThreadAlreadyEnabledException,
        ...
        Class<?> clazz = Class.forName(classNameNormalized, true, Thread.currentThread().getContextClassLoader());
