 ...
 * This class should be used for all access to the trace package.
 */
public class SymbolicTraceHandler {
     ...
     * This is used to catch all unsupported branching behaviour.
     ...
     */
    public HashMap<Integer, BooleanFormula> getPathConstraints(int iid) throws IllegalArgumentException{
        HashMap<Integer, BooleanFormula> constraints = new HashMap<>();
        for (Element elem : symbolicTrace.getTrace()) {
            if (elem instanceof BranchElement b) {
                if(b.getIid() == iid) {
     ...
     * Each symbolic input has two bounds, the lower and the upper bound of the respective datatype.
