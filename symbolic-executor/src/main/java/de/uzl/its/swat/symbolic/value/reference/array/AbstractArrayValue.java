...
public abstract class AbstractArrayValue<
    ...
    /**
     * Returns a JSON-encoded string representation of the concrete array value.
     * This is used when building the trace DTO to send to the explorer.
     *
     * @return JSON string representation of the array (e.g., "[1, 2, 3]")
     */
    @Override
    public String getConcreteEncoded() {
        ...
        // Use Java's built-in Arrays.toString() which produces the format we need
        // For primitive arrays: "[1, 2, 3]"
        // For object arrays, we'll need to handle differently
        if (concrete instanceof int[]) {
            return java.util.Arrays.toString((int[]) concrete);
        } else if (concrete instanceof boolean[]) {
            return java.util.Arrays.toString((boolean[]) concrete);
        } else if (concrete instanceof byte[]) {
            return java.util.Arrays.toString((byte[]) concrete);
        } else if (concrete instanceof char[]) {
            return java.util.Arrays.toString((char[]) concrete);
        } else if (concrete instanceof short[]) {
            return java.util.Arrays.toString((short[]) concrete);
        } else if (concrete instanceof long[]) {
            return java.util.Arrays.toString((long[]) concrete);
        } else if (concrete instanceof float[]) {
            return java.util.Arrays.toString((float[]) concrete);
        } else if (concrete instanceof double[]) {
            return java.util.Arrays.toString((double[]) concrete);
        } else if (concrete instanceof Object[]) {
            return java.util.Arrays.toString((Object[]) concrete);
        } else {
            return "[]";
        }
    }
