...
public class Config {
    ...
    /** Flag to determine if heuristic delegation detection should be used. */
    @Getter private boolean useDelegationDetection;
    private static final boolean DEFAULT_USE_DELEGATION_DETECTION = true;
    ...
    // ------------------------------------
    // Symbolic array options
    // ------------------------------------
    /** Maximum length for symbolic arrays to prevent unbounded array sizes during exploration. */
    @Getter private int maxArrayLength;
    private static final int DEFAULT_MAX_ARRAY_LENGTH = 100;
    ...
    private boolean readBoolean(String key, boolean defaultValue) {
        ...
        if (cmdLineValue != null) {
            ...
            logger.debug("[CommandLine] " + key + ":" + val);
            return val;
            ...
            logger.debug("[Default] " + key + ":" + defaultValue);
            return defaultValue;
        }
    }
    ...
    private String readString(String key, String defaultValue) {
        String cmdLineValue = System.getProperty(key);
        if (cmdLineValue != null) {
            logger.debug("[CommandLine] " + key + ":" + cmdLineValue);
            return cmdLineValue;
        } else if (properties.containsKey(key)) {
    ...
    private String[] readList(String key, String[] defaultValue) {
        ...
        if (cmdLineValue != null) {
            ...
            logger.debug("[CommandLine] " + key + ":" + Arrays.toString(val));
            return val;
        } else if (properties.containsKey(key)) {
            ...
            logger.debug("[Default] " + key + ":" + Arrays.toString(defaultValue));
            return defaultValue;
        }
    }
    ...
    private void readProperties() {
        ...
        useDelegationDetection = readBoolean("useDelegationDetection", DEFAULT_USE_DELEGATION_DETECTION);
        ...
        // Symbolic array options
        // ------------------------------------
        maxArrayLength = readInt("maxArrayLength", DEFAULT_MAX_ARRAY_LENGTH);
