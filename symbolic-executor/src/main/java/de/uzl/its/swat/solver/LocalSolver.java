...
/**
 * This class can is used to directly communicate with the solver (currently Z3). The solver interface only tries to find
 * a solution for each branch (with the corresponding path constraints) and log it. It cannot be used to perform iterative symbolic execution and is primarily intended for debugging and testing purposes.
 *
 */
public class LocalSolver {
     ...
     * Then, it tries to find a solution for each constraint and logs it.
     */
    public static void solve() {
        ...
        for(Map.Entry<Integer, BooleanFormula> entry: constraints.entrySet()){
            ...
            // The constraint is negated to find a solution leading the opposite branching direction.
            ...
            if (!freeVars.isEmpty()) {
                try (ProverEnvironment prover =
                    ...
                    HashMap<Integer, BooleanFormula> pathConstraints = symbolicTraceHandler.getPathConstraints(entry.getKey());
                    ...
                    logger.info("--------------------------------------------------------------");
                    logger.info("[Endpoint] " + ThreadHandler.getEndpointName(currentThread().getId()));
                    symbolicTraceHandler.dumpInputs(logger);
                    logger.info("[Constraint] " + fmgr.dumpFormula(constraint).toString().replaceAll("[\\r\\n]", ""));
                    if (!isUnsat) {
