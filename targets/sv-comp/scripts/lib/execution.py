...
def target_execution(ver_task: VerificationTask) -> tuple[Path, str, int, ExecutionStatus, bool, Optional[bool], float, dict]:
    ...
    Returns:
        Tuple of (target, case, points, execution_status, error, validated, elapsed_time_seconds, timing_breakdown)
    """
    ...
    cmd = command["command"]
    validated = None
    witness_generation_time = 0.0
    witness_validation_time = 0.0
    ...
    # Try to read timing data from the symbolic explorer's output file
    timing_breakdown = {}
    timing_file = log_dir / 'timing_data.json'
    if timing_file.exists():
        try:
            with open(timing_file, 'r') as f:
                timing_data = json.load(f)
                timing_breakdown = timing_data.get('aggregates', {})
                # Update witness timing from our measurements
                timing_breakdown['witness_generation'] = witness_generation_time
                timing_breakdown['witness_validation'] = witness_validation_time

            # Write the updated timing data back to the file
            timing_data['aggregates'] = timing_breakdown
            with open(timing_file, 'w') as f:
                json.dump(timing_data, f, indent=2)
                logger.debug(f"Updated timing data in {timing_file} with witness timing")
        except Exception as e:
            logger.warning(f"Could not read timing data from {timing_file}: {e}")
            timing_breakdown = {
                'witness_generation': witness_generation_time,
                'witness_validation': witness_validation_time
            }
    else:
        timing_breakdown = {
            'witness_generation': witness_generation_time,
    ...
    ci_print(f"::endgroup::")
    return target, case, points, execution_status, error, validated, elapsed_time, timing_breakdown
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_task): # type: ignore
            try:
                ...
                target, case, points, execution_status, error, validated, elapsed_time, timing_breakdown = future.result()
                logger.info(f"{ver_task['category'].value} | Points: {points}, Case: {case}, Execution Status: {execution_status}, Error: {error}, Validated: {validated}, Time: {elapsed_time:.2f}s")

                results[ver_task['category'].value][target] = (case, points, execution_status.value, error, validated, elapsed_time, timing_breakdown)
            except Exception as e:
...
def evaluate_results(results):
    ...
    for category_name, category_results in results.items():
        ...
        for target, result_tuple in category_results.items():
            # Handle old formats and new format with timing breakdown
            if len(result_tuple) == 7:
                case, points, execution_status, error, validated, elapsed_time, timing_breakdown = result_tuple
            ...
            for stage, stage_time in timing_breakdown.items():
                if stage in stats['stage_timing']:
                    stats['stage_timing'][stage] += stage_time
        ...
        stage_timing = category_stats[category_name]['stage_timing']
        ...
        if stage_total > 0:
            ...
            logger.info(f"  - Witness Generation:  {stage_timing['witness_generation']:>8.2f}s ({stage_timing['witness_generation']/stage_total*100:>5.1f}%)")
            logger.info(f"  - Witness Validation:  {stage_timing['witness_validation']:>8.2f}s ({stage_timing['witness_validation']/stage_total*100:>5.1f}%)")
            logger.info(f"  - Total Stage Time:    {stage_total:>8.2f}s")
    ...
    for category_name, stats in category_stats.items():
        for stage, time_val in stats['stage_timing'].items():
            aggregate_stage_timing[stage] += time_val

    aggregate_total = sum(aggregate_stage_timing.values())
    if aggregate_total > 0:
        ...
        logger.info(f"  - Witness Generation:  {aggregate_stage_timing['witness_generation']:>8.2f}s ({aggregate_stage_timing['witness_generation']/aggregate_total*100:>5.1f}%)")
        logger.info(f"  - Witness Validation:  {aggregate_stage_timing['witness_validation']:>8.2f}s ({aggregate_stage_timing['witness_validation']/aggregate_total*100:>5.1f}%)")
        logger.info(f"{'='*50}")
...
def save_results(category_stats, results, folder='results'):
    ...
    for category_name, stats in category_stats.items():
        ...
        data = {
            ...
            'timing': timing_stats,
            'stage_timing': stats['stage_timing'],
...
def save_aggregate_stage_timing(category_stats, folder='results'):
    ...
    with open(filepath, 'w') as f:
        # Header
        f.write("Stage,Time (seconds),Percentage\n")
    ...
    logger.info(f"Aggregate stage timing saved to {filepath}")
...
def run_single_target(ver_tasks: list[VerificationTask]):
    ...
    if result and len(result) >= 8:
        _, _, _, _, _, _, _, timing_breakdown = result
        if timing_breakdown:
            logger.info("\n" + "="*60)
